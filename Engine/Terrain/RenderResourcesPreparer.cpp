...
namespace Cesium
{
    ...
    void RenderResourcesPreparer::OnTick([[maybe_unused]] float deltaTime, [[maybe_unused]] double time)
    {
   ...
   //     }
    ...
    bool RenderResourcesPreparer::AddRasterLayer(const Cesium3DTilesSelection::RasterOverlay* rasterOverlay)
    {
        ...
        {
            m_rasterOverlayLayers.insert(std::make_pair(rasterOverlay, m_freeRasterLayers.back()));
    ...
    /*    if (pLoadThreadResult)
        {
            ...
            std::unique_ptr<GltfLoadModel> loadModel{ reinterpret_cast<GltfLoadModel*>(pLoadThreadResult) };
    ...
    void* RenderResourcesPreparer::prepareRasterInLoadThread(const CesiumGltf::ImageCesium& image, const std::any& rendererOptions)
    {
  ...
  /*      if (pLoadThreadResult)
   ...
   /*             IntrusiveGltfModel* intrusiveGltfModel = reinterpret_cast<IntrusiveGltfModel*>(tileRenderResource);
    ...
    std::optional<glm::dvec3> RenderResourcesPreparer::GetRTCFromGltf(const CesiumGltf::Model& model)
    {
