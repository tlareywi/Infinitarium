...
namespace Cesium
{
  ...
  /*  struct IntrusiveGltfModel
    ...
    public:
        ...
        void OnTick(float deltaTime, double time);
    ...
    private:
        std::optional<glm::dvec3> GetRTCFromGltf(const CesiumGltf::Model& model);
     ...
     //   AZStd::vector<AZ::Data::Instance<AZ::RPI::Material>> m_compileMaterialsQueue;
        std::map<const Cesium3DTilesSelection::RasterOverlay*, std::uint32_t> m_rasterOverlayLayers;
        std::vector<std::uint32_t> m_freeRasterLayers;
    };
} // namespace Cesium
