...
namespace Cesium
{
        ...
        : m_httpManager{ httpManager }
    {
        ...
        std::string platformName = "Win64";
        std::string projectName = "Demo";
        m_userAgentHeaderValue = std::string("Mozilla/5.0 (") + platformName.c_str() + ") Infinitarium/" +
            engineVersion + " (Project " + projectName.c_str() + " Infinitarium Engine " + engineVersion + ")";
    }
        ...
        const CesiumAsync::AsyncSystem& asyncSystem, const std::string& url, const std::vector<THeader>& headers)
    {
        ...
        HttpRequestParameter parameter(std::string(url.c_str()), Aws::Http::HttpMethod::HTTP_GET, std::move(requestHeaders));
        ...
        const gsl::span<const std::byte>& contentPayload)
    {
        CesiumAsync::HttpHeaders requestHeaders = ConvertToCesiumHeaders(headers);
        requestHeaders[USER_AGENT_HEADER_KEY] = m_userAgentHeaderValue;
        std::string requestBody(reinterpret_cast<const char*>(contentPayload.data()), contentPayload.size());
        HttpRequestParameter parameter(
            std::string(url.c_str()), Aws::Http::HttpMethod::HTTP_POST, std::move(requestHeaders), std::move(requestBody));
        return m_httpManager->AddRequest(asyncSystem, std::move(parameter))
            .thenImmediately(
                [](HttpResult&& result) -> std::shared_ptr<CesiumAsync::IAssetRequest>
                {
