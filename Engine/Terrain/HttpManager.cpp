...
namespace Cesium
{
        ...
        const CesiumAsync::AsyncSystem& asyncSystem, HttpRequestParameter&& httpRequestParameter)
    {
        ...
        std::function<void()>* job = new std::function<void()>(
            RequestHandler{ m_awsHttpClient, std::move(httpRequestParameter), promise } );
        std::thread(job).detach();

        return promise.getFuture();
    }

    std::string HttpManager::GetParentPath(const std::string& path)
    {
        ...
        const CesiumAsync::AsyncSystem& asyncSystem, const IORequestParameter& request)
    {
        ...
        std::function<void()>* job = new std::function<void()>(
            GenericIORequestHandler{ m_awsHttpClient, request, promise } );
        std::thread(job).detach();
        ...
        const CesiumAsync::AsyncSystem& asyncSystem, IORequestParameter&& request)
    {
        auto promise = asyncSystem.createPromise<IOContent>();
        std::function<void()>* job = new std::function<void()>(
            GenericIORequestHandler{ m_awsHttpClient, std::move(request), promise } );
        std::thread(job).detach();
